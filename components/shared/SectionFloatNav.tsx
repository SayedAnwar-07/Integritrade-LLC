...
export type FloatNavSection = {
  id: string;
  label: string;
};

// Sticky numbered stepper that tracks which section is in view, mirroring the
// home-page nav. Pass the sections that exist on the page; each `id` must match
// an element id so the observer can follow it.
export default function SectionFloatNav({
  sections,
  className = "sticky top-28",
}: {
  sections: FloatNavSection[];
  /** Wrapper classes — override to float the rail instead of sitting in-flow. */
  className?: string;
}) {
  ...
  // Measure the real pixel distance to the active dot so the progress line
  // always lands exactly on-center, regardless of spacing/font changes.
  useEffect(() => {
    const measure = () => {
    ...
    <aside className={className}>
      <div ref={listRef} className="relative flex flex-col">
        ...
        <div
          ...
          style={{
            top: trackBounds.top,
            height: Math.max(trackBounds.bottom - trackBounds.top, 0),
          }}
        />
        ...
        {sections.map((section, index) => {
          ...
          return (
            <button
              ...
              className="group relative z-10 flex items-center gap-4 py-4 text-left outline-none"
            >
              {/* Circle */}
              <span
                ref={(el) => {

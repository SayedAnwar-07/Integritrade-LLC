 ...
 * article drafts (2026-07-21). Deterministic [City] substitution only — no
 * content is rewritten. Per-city SEO fields (title, shortDescription, icon,
 * metaTitle, metaDescription, ctaText) of EXISTING services are preserved
 * from the current files; heroHeading, heroSubheading and article details
 * are replaced with the new approved copy.
 *
 * Adds one NEW service per city: it-equipment-remarketing.
 *
...
const HARD_DRIVE_SHREDDING = {
  slug: "data-destruction-services", // default for cities missing this service; existing slug is preserved per city
...
const ELECTRONICS_RECYCLING = {
  slug: "basic-electronics-recycling", // renamed by Tamim 2026-07-22 (was e-waste-recycling); existing slug is preserved per city
...
/** Deep-clone a template, replacing {{CITY}} in every string. */
function fillCity(value, cityName) {
  if (typeof value === "string") return value.replaceAll("{{CITY}}", cityName);
  if (Array.isArray(value)) return value.map((v) => fillCity(v, cityName));
  if (value && typeof value === "object") {
    const out = {};
    for (const [k, v] of Object.entries(value)) out[k] = fillCity(v, cityName);
    return out;
  }
  return value;
}
...
/**
 * Article categories mapped onto whatever slug each city historically uses.
 * Existing slugs are KEPT (they are live, sitemap-indexed URLs); the template
 * slug is only used when a city doesn't have that service yet.
 */
...
for (const file of files) {
  ...
  for (const orig of originalSlugs) {
    ...
    if (assigned.some((a) => a.cat === cat)) {
      warnings.push(`${file}: duplicate category for slug "${orig}" — skipped`);
      continue;
    }
    assigned.push({ cat, slug: orig });
  }
  // Append categories this city doesn't have yet, using canonical slugs
  for (const cat of CATEGORIES) {
    if (!assigned.some((a) => a.cat === cat)) {
      assigned.push({ cat, slug: cat.template.slug, added: true });
    }
  }

  for (const a of assigned) {
    if (a.added) addedUrls.push(`/service-area/${area.slug}/${a.slug}/`);
  }

  const services = assigned.map(({ cat, slug }) => {
    const filled = fillCity(cat.template, cityName);
    return { ...filled, slug };
  });
  ...
  const out = `// AUTO-GENERATED by scripts/generate-city-articles.mjs — do not edit article
...
if (addedUrls.length) {
  ...
  for (const u of addedUrls) console.log("  + " + u);
}
if (warnings.length) {
  console.log("\nWarnings:");
